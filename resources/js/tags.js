...
function addTag(tag) {
    console.log("ADDING...");
    console.log(tag);
    // gather elements to input, display, and submit to the form
    var tagInput = document.createElement('input');
    var resourceTags = document.getElementById('resource-tags');
    var resourceTagIds = document.getElementById('resource-tag-ids');

    // create the input element
    tagInput.type = 'hidden';
    tagInput.name = 'tags[' + resourceTags.childElementCount + '][id]';
    tagInput.value = tag.dataset.tid;
    tagInput.classList.add('resource-tag-id');

    // add tags to respective containers
    newTag = resourceTags.appendChild(tag);
    resourceTagIds.appendChild(tagInput);

    // link remove function to new tag
...
function removeTag(tag, tagInput) {
    ...
    // remove id from resource tag ids collection
    resourceTagIds.removeChild(tagInput);

    // move tag from resource tag collection to tag container collection
    oldTag = tagContainer.appendChild(tag);

    // change function to add tag
    oldTag.onclick = addTag.bind(this, oldTag);

    // reset index on names of resource tag ids
    resetTagIndices();
}
 ...
 ********************************************************************/
function resetTagIndices(collectionClass="resource-tag-id") {
