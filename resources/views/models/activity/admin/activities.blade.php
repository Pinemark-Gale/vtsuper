            ...
            @foreach ($activities as $activity)
                <tr class="base-row bottom-border">
                    <th class="first-col" scope="row">
                        <a href="{{ route('admin-activity-edit', ['activity' => $activity->name]) }}">{{ $activity->name }}</a>
                    </th>
                ...
                <tr class="hidden bottom-border">
                    ...
                    <td class="second-col">
                        ...
                        <div class="action-container">
                            <a href="{{ route('admin-activity', ['activity' => $activity->name]) }}">
                                <button class="action-button" data-message="Get more info on {{ $activity->name }} activity.">More Info</button>
                            </a>
                            <form method="POST" action="{{ route('admin-activity-destroy', ['activity' => $activity->name]) }}">
                                @csrf
                                @method('DELETE')
